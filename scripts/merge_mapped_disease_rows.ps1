...
param(
    ...
    [string]$MappingResultSheet = 'Mapping结果',
    [string]$FinalSheetName = 'MergedMappedPool',
    [Parameter(Mandatory = $true)] [string]$SourceKeyColumn,
    [string]$MappingIdColumn = 'Mapping ID',
    [Parameter(Mandatory = $true)] [string]$EntityColumn,
    [Parameter(Mandatory = $true)] [string]$IndicationColumn,
    [string]$DiseaseColumn = '对应疾病',
    [string]$ResultDiseaseColumn = 'Disease CN',
    [string]$StatusColumn = 'Mapping Status',
    [string]$RationaleColumn = 'Rationale',
...
try {
    ...
    try { $mapping = $sourceBook.Worksheets.Item($MappingResultSheet) }
    catch { throw "Mapping-result sheet was not found: $MappingResultSheet" }

    $rowCount = [int]$source.UsedRange.Rows.Count
    $columnCount = [int]$source.UsedRange.Columns.Count
    $mappingRowCount = [int]$mapping.UsedRange.Rows.Count
    $mappingColumnCount = [int]$mapping.UsedRange.Columns.Count
    if ($rowCount -lt 2) { throw 'Source sheet has no data rows.' }
    if ($mappingRowCount -lt 2) { throw 'Mapping-result sheet has no data rows.' }
    ...
    $mappingRange = $mapping.Range($mapping.Cells.Item(1, 1), $mapping.Cells.Item($mappingRowCount, $mappingColumnCount))
    ...
    for ($column = 1; $column -le $columnCount; $column++) {
        ...
        if (-not $header -or $headers.ContainsKey($header)) { throw "Source header is blank or duplicated at column $column." }
        $headers[$header] = $column
        $headerNames[$column] = $header
    }
    $requiredSource = @($SourceKeyColumn, $MappingIdColumn, $EntityColumn, $IndicationColumn, $DiseaseColumn, $GenericColumn, $DosageFormColumn, $BrandColumn)
    foreach ($header in $requiredSource) {
        if (-not $headers.ContainsKey($header)) { throw "Source sheet does not contain required field: $header" }
    }

    if ($mappingColumnCount -ne $ExpectedMappingHeaders.Count) {
        throw "Mapping-result sheet must contain exactly $($ExpectedMappingHeaders.Count) columns."
    }
    $mappingHeaders = @{}
    for ($column = 1; $column -le $mappingColumnCount; $column++) {
        $header = ([string]$mappingValues[1, $column]).Trim()
        if ($header -ne $ExpectedMappingHeaders[$column - 1]) {
            throw "Mapping-result header mismatch at column $column. Expected '$($ExpectedMappingHeaders[$column - 1])', found '$header'."
        }
        $mappingHeaders[$header] = $column
    }
    foreach ($header in @($MappingIdColumn, $ResultDiseaseColumn, $StatusColumn, $RationaleColumn)) {
        if (-not $mappingHeaders.ContainsKey($header)) { throw "Mapping-result sheet does not contain required field: $header" }
    }

    $culture = [System.Globalization.CultureInfo]::InvariantCulture
    function Convert-CellText {
        param([object]$Value, [string]$Header)
    ...
    function Get-SourceCellText {
        param([int]$Row, [int]$Column)
        return Convert-CellText -Value $sourceValues[$Row, $Column] -Header $headerNames[$Column]
    }
    function Get-SourceField {
        param([int]$Row, [string]$Header)
        return Get-SourceCellText -Row $Row -Column ([int]$headers[$Header])
    }
    function Get-MappingField {
        param([int]$Row, [string]$Header)
        return Convert-CellText -Value $mappingValues[$Row, [int]$mappingHeaders[$Header]] -Header $Header
    }

    $sourceFormulaCount = 0
    $sourceExternalFormulaCount = 0
    for ($row = 1; $row -le $rowCount; $row++) {
        for ($column = 1; $column -le $columnCount; $column++) {
            $formula = [string]$sourceFormulas[$row, $column]
            if ($formula.StartsWith('=')) {
                $sourceFormulaCount++
                if ($formula.Contains('[')) { $sourceExternalFormulaCount++ }
            }
        }
    }
    $mappingFormulaCount = 0
    $mappingExternalFormulaCount = 0
    for ($row = 1; $row -le $mappingRowCount; $row++) {
        for ($column = 1; $column -le $mappingColumnCount; $column++) {
            $formula = [string]$mappingFormulas[$row, $column]
            if ($formula.StartsWith('=')) {
                $mappingFormulaCount++
                if ($formula.Contains('[')) { $mappingExternalFormulaCount++ }
            }
        }
    }
    ...
    $statusCounts = @{}
    for ($row = 2; $row -le $mappingRowCount; $row++) {
        $mappingId = Get-MappingField -Row $row -Header $MappingIdColumn
        if (-not $mappingId -or $mappingById.ContainsKey($mappingId)) { throw "Mapping-result ID is blank or duplicated: $mappingId" }
        $disease = Get-MappingField -Row $row -Header $ResultDiseaseColumn
        $status = Get-MappingField -Row $row -Header $StatusColumn
        $rationale = Get-MappingField -Row $row -Header $RationaleColumn
        if (-not $status) { throw "Mapping Status is blank for Mapping ID $mappingId." }
        if (-not $rationale) { throw "Rationale is blank for Mapping ID $mappingId." }
        if ($status -eq $MappedValue -and -not $disease) { throw "Mapped result has blank disease for Mapping ID $mappingId." }
        if ($status -ne $MappedValue -and $disease) { throw "Non-mapped result has a disease for Mapping ID $mappingId." }
    ...
    for ($row = 2; $row -le $rowCount; $row++) {
        $mappingId = Get-SourceField -Row $row -Header $MappingIdColumn
        if (-not $mappingId -or $sourceIds.ContainsKey($mappingId)) { throw "Source Mapping ID is blank or duplicated: $mappingId" }
        if (-not $mappingById.ContainsKey($mappingId)) { throw "Source Mapping ID is absent from Mapping results: $mappingId" }
        $sourceIds[$mappingId] = $row
        [void]$sourceOrder.Add($mappingId)
        $joined = $mappingById[$mappingId]
        $backfill = Get-SourceField -Row $row -Header $DiseaseColumn
        $expectedBackfill = if ($joined.Status -eq $MappedValue) { $joined.Disease } else { '' }
        if ($backfill -ne $expectedBackfill) {
            throw "Source disease backfill does not match Mapping results for Mapping ID $mappingId."
        }
    }
    if ($sourceIds.Count -ne $mappingById.Count) { throw 'Source and Mapping-result ID counts differ.' }
    if ([string]::Join($GroupSeparator, $sourceOrder.ToArray()) -ne [string]::Join($GroupSeparator, $mappingOrder.ToArray())) {
        throw 'Source and Mapping-result ID order differs.'
    }
    ...
    for ($row = 2; $row -le $rowCount; $row++) {
        ...
        $sourceKey = Get-SourceField -Row $row -Header $SourceKeyColumn
        $entity = Get-SourceField -Row $row -Header $EntityColumn
    ...
    foreach ($entry in $layerOne) {
        $entry.MappingIds = Join-Unique @($entry.Rows | ForEach-Object { Get-SourceField -Row $_ -Header $MappingIdColumn })
        $entry.Indications = Join-Unique @($entry.Rows | ForEach-Object { Get-SourceField -Row $_ -Header $IndicationColumn })
        $entry.Rationales = Join-Unique @($entry.Rows | ForEach-Object {
            $id = Get-SourceField -Row $_ -Header $MappingIdColumn
            $mappingById[$id].Rationale
        })
    }
    ...
    foreach ($entry in @($layerOne | Where-Object { $_.Rows.Count -gt 1 })) {
        foreach ($field in @($EntityColumn, $GenericColumn, $DosageFormColumn, $BrandColumn)) {
            $values = @(Get-UniqueArray @($entry.Rows | ForEach-Object { Get-SourceField -Row $_ -Header $field }))
    ...
    foreach ($entry in $layerOne) {
        if ($entry.Status -eq $MappedValue -and $entry.Disease) {
            $key = @(
                $entry.Entity,
                (Get-SourceField -Row $entry.Anchor -Header $GenericColumn),
                (Get-SourceField -Row $entry.Anchor -Header $DosageFormColumn),
                (Get-SourceField -Row $entry.Anchor -Header $BrandColumn),
                $entry.Indications,
                $entry.Disease,
                $entry.Status
            ) -join $GroupSeparator
        }
        else { $key = "ROW$GroupSeparator$($entry.Anchor)" }
    ...
    $previewLayerTwo = @($layerTwoGroups | ForEach-Object {
        ...
        for ($column = 1; $column -le $columnCount; $column++) {
            $values = @(Get-UniqueArray @($_.Children | ForEach-Object { Get-SourceCellText -Row $_.Anchor -Column $column }))
        ...
        [pscustomobject]@{
            Entity = (Get-SourceField -Row $_.Anchor -Header $EntityColumn)
            Disease = $_.Disease
            SourceKeys = (Join-Unique @($_.Children | ForEach-Object { $_.SourceKey }))
            MappingIds = (Join-Unique @($_.Children | ForEach-Object { $_.MappingIds }))
            Indications = (Join-Unique @($_.Children | ForEach-Object { $_.Indications }))
            RegistrationDifferenceFields = ($differenceFields -join ', ')
            ReviewDistinctRationale = ($rationales.Count -gt 1)
        }
    })
    ...
    $preview = [ordered]@{
        ...
        MappingResultRows = $mappingRowCount - 1
        MappingJoinValidated = $true
        MappingStatusCounts = $statusCounts
        SourceFormulaCells = $sourceFormulaCount
        SourcePotentialExternalFormulaCells = $sourceExternalFormulaCount
        MappingFormulaCells = $mappingFormulaCount
        MappingPotentialExternalFormulaCells = $mappingExternalFormulaCount
    ...
    function Get-OutputValue {
        ...
        if ($header -eq $DiseaseColumn) { return $Final.Disease }
        if ($Final.Children.Count -eq 1) { return Get-SourceCellText -Row $Final.Children[0].Anchor -Column $Column }
        return Join-Unique @($Final.Children | ForEach-Object { Get-SourceCellText -Row $_.Anchor -Column $Column })
    }
    ...
    $copiedMapping = $outputBook.Worksheets.Item($MappingResultSheet)
    $copiedSourceFormulas = $copiedSource.Range($copiedSource.Cells.Item(1, 1), $copiedSource.Cells.Item($rowCount, $columnCount)).Formula
    $copiedMappingFormulas = $copiedMapping.Range($copiedMapping.Cells.Item(1, 1), $copiedMapping.Cells.Item($mappingRowCount, $mappingColumnCount)).Formula
    for ($row = 1; $row -le $rowCount; $row++) {
        for ($column = 1; $column -le $columnCount; $column++) {
            if ([string]$sourceFormulas[$row, $column] -ne [string]$copiedSourceFormulas[$row, $column]) { throw "Copied source sheet changed at R$row C$column." }
        }
    }
    for ($row = 1; $row -le $mappingRowCount; $row++) {
        for ($column = 1; $column -le $mappingColumnCount; $column++) {
            if ([string]$mappingFormulas[$row, $column] -ne [string]$copiedMappingFormulas[$row, $column]) { throw "Copied Mapping-result sheet changed at R$row C$column." }
        }
    }
    ...
    for ($outputRow = 1; $outputRow -le $outputRows; $outputRow++) {
        ...
        for ($column = 1; $column -le $columnCount; $column++) {
            $matrix.SetValue(([string](Get-OutputValue -Final $entry -Column $column)), $outputRow, $column)
        }
    }
    ...
    for ($row = 1; $row -le ($outputRows + 1); $row++) {
        for ($column = 1; $column -le $columnCount; $column++) {
            if (([string]$finalFormulas[$row, $column]).StartsWith('=')) { $finalFormulaCount++ }
        }
    }
    ...
    [pscustomobject]@{
        ...
        MappingResultSheetPreserved = $true
